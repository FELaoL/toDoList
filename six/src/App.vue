<template>
	<div class="dialog" id="app">
		...
		<ul class="todos">
			<li :class="{ completed: item.completed }" class="todos-item" v-for="(item, index) in infoList" :key="index">
				<!-- 复选框 -->
				<input type="checkbox" class="checkbox-btn" :checked="item.completed" @click="e => changeCheckBox(e, item.id)" />
			...
			<div class="out" v-else>
				...
				<div class="filters">
					...
					<span
						...
						v-if="completeCount > 0 && unDoneLength > 0"
				...
				<!-- 把已完成的任务清空 -->
				<span class="option option3" v-if="completeCount > 0" @click="clearCompleted">Clear completed</span>
			</div>
		</div>
	</div>
</template>
